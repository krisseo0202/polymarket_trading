...
class BTCUpDownStrategy(Strategy):
    ...
    def analyze(self, market_data: Dict[str, Any]) -> List[Signal]:
        ...
        exit_sig = self._check_exit(order_books, by_token, now_ts)
        if exit_sig:
            self._reset_position_state()
            return [exit_sig]
    ...
    def _check_exit(
        ...
        Exit conditions (priority order):
          1. profit_pct  >= profit_target_pct  → take-profit
          2. -profit_pct >= stop_loss_pct       → stop-loss
          3. time_held   >= max_hold_seconds    → time-limit
        """
        ...
        if not held_size:
            self._reset_position_state()   # closed externally
            return None
